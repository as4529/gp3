...
class MFSVI(SVIBase):
    ...
    def grad_kern(self):
        ...
        for i, X in enumerate(reversed(self.X_dims)):
            ...
            toep_grad = np.stack([toeplitz(grad[:, :, k]) for k in range(grad.shape[2])],
                                 axis = -1)
        ...
        for grad in grads:
            ...
            term3 = np.stack([np.dot(grad[:,:,d], term3[:,:,d]).T for
                              d in range(len(self.kernel.params))], axis = -1)

        term3 = np.reshape(term3, [-1, len(self.kernel.params)])
    ...
    def nat_grad_kern(self):
        ...
        fisher = np.zeros((len(self.kernel.params), len(self.kernel.params)))

        for i in range(len(self.kernel.params)):
            for j in range(len(self.kernel.params)):
                if fisher[i,j] != 0:
                    continue
